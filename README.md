...
| :--- | :--- | :--- | :--- |
| **🌐 Riot 官方網站安全跳轉登入 (推薦)** | **電腦網頁版 / 手機 / 跨裝置** | 點擊後開啟原廠 `auth.riotgames.com` 頁面（帶有 `prompt=login` 強制登入），登入完成後自動跳轉回本站 Public 網址；若在手機端，登入後只要**複製網址列**並切回本站即可自動讀取剪貼簿秒速載入！ | 🛡️ **最高安全**（密碼全程直接在原廠網頁鍵入，絕不接觸第三方伺服器） |
