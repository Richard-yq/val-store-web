...
本專案全面採用 **Riot 官方原廠授權機制**，確保零帳密外洩風險：

| 登入方式 | 適用情境 | 運作原理 | 安全性 |
| :--- | :--- | :--- | :--- |
| **🌐 Riot 官方網站安全跳轉登入 (推薦)** | **網頁版 / 手機版 / 跨裝置** | 點擊後開啟原廠 `auth.riotgames.com` 頁面，**強制帶有 `prompt=login` 參數**，每次皆要求手動輸入帳密與 2FA 驗證碼，登入後自動帶回 Access Token 載入商城。 | 🛡️ **最高**（帳密全程直接在 Riot 原廠網頁輸入，密碼絕不接觸或儲存於任何第三方伺服器） |
